...
public class TestTable {
  ...
  /**
   * Tests RIDBlockIterator over two pages of records with gaps, and an empty
   * middle page.
   */
  // HIDDEN TEST
  @Test
  public void testRIDBlockIteratorWithGaps() throws DatabaseException {
    List<RecordId> rids = new ArrayList();
    int numRecords = setupIteratorTest(rids, 3);

    // Delete every other record and the last record of the first page.
    for (int i = 0; i < numRecords / 3 - 1; i += 2) {
      table.deleteRecord(rids.get(i));
    }
    table.deleteRecord(rids.get(numRecords / 3 - 1));

    // Delete every record of the second page.
    for (int i = numRecords / 3; i < 2 * numRecords / 3; ++i) {
      table.deleteRecord(rids.get(i));
    }

    // Delete every other record and the last record of the third page.
    for (int i = 2 * numRecords / 3; i < numRecords - 1; i += 2) {
      table.deleteRecord(rids.get(i));
    }
    table.deleteRecord(rids.get(numRecords - 1));
    ...
    BacktrackingIterator<Record> iter = new RecordIterator(table, table.new RIDBlockIterator(pages, 3));
    checkSequentialRecords(1, numRecords / 3 - 1, 2, iter);
    checkSequentialRecords(2 * numRecords / 3 + 1, numRecords - 1, 2, iter);
    assertFalse(iter.hasNext());
  }
  ...
  /**
   * Extra test making sure that RIDBlockIterator handles mark/reset properly.
   */
  // HIDDEN TEST
  @Test
  public void testRIDBlockIteratorMarkResetExtra() throws DatabaseException {
    int numRecords = setupIteratorTest(3);
    Iterator<Page> pages = table.getAllocator().iterator();
    // Header page.
    pages.next();
  ...
  // HIDDEN TEST
  @Test
  public void testRIDBlockIteratorMarkResetWithGaps() throws DatabaseException {
    List<RecordId> rids = new ArrayList();
    int numRecords = setupIteratorTest(rids, 3);
    ...
    int stop = numRecords / 6;
    if (stop % 2 == 0) {
      ++stop;
    }
    checkSequentialRecords(1, stop, 2, iter);
    iter.mark();
    checkSequentialRecords(stop, numRecords / 3 - 1, 2, iter);
    checkSequentialRecords(2 * numRecords / 3 + 1, numRecords - 1, 2, iter);
    assertFalse(iter.hasNext());
    iter.reset();
    // -2 because the record before the mark must also be returned
    checkSequentialRecords(stop - 2, numRecords / 3 - 1, 2, iter);
    checkSequentialRecords(2 * numRecords / 3 + 1, numRecords - 1, 2, iter);
    assertFalse(iter.hasNext());
    ...
    pages.next();
    iter = new RecordIterator(table, table.new RIDBlockIterator(pages, 3));
    // test second to last record of first page
    checkSequentialRecords(1, numRecords / 3 - 3, 2, iter);
    iter.mark();
    checkSequentialRecords(numRecords / 3 - 3, numRecords / 3 - 1, 2, iter);
    checkSequentialRecords(2 * numRecords / 3 + 1, numRecords - 1, 2, iter);
    assertFalse(iter.hasNext());
    iter.reset();
    checkSequentialRecords(numRecords / 3 - 5, numRecords / 3 - 1, 2, iter);
    checkSequentialRecords(2 * numRecords / 3 + 1, numRecords - 1, 2, iter);
    assertFalse(iter.hasNext());
    iter.reset();
    iter.next();
    iter.next();
    // test last record of first page
    iter.mark();
    checkSequentialRecords(2 * numRecords / 3 + 1, numRecords - 1, 2, iter);
    assertFalse(iter.hasNext());
    iter.reset();
  ...
  // HIDDEN TEST
  @Test
  public void testRIDBlockIteratorMarkResetExtraWithGaps() throws DatabaseException {
    List<RecordId> rids = new ArrayList();
    int numRecords = setupIteratorTest(rids, 3);
